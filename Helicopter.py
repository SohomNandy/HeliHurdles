...
font = pygame.font.Font('NeueHaasDisplayBold.ttf', 20)
fps = 60
...
highscore = 0
active = True
heli = pygame.transform.scale(pygame.image.load('helicopter.png'), (60, 40))
...
def draw_map(rects):
    for i in range(len(rects)):
        pygame.draw.rect(screen, 'forest green', rects[i])
    pygame.draw.rect(screen, 'dark gray', [0, 0, WIDTH, HEIGHT], 12)


def draw_player():
    # draw player hit circumference as circle and import helicopter image
    if score <= 500:
        player = pygame.draw.circle(screen, 'peru', (player_x, player_y), 15)
    elif score <= 1000:
        player = pygame.draw.circle(screen, 'chocolate', (player_x, player_y), 15)
    elif score <= 1500:
        player = pygame.draw.circle(screen, 'sienna', (player_x, player_y), 15)
    else:
        player = pygame.draw.circle(screen, 'black', (player_x, player_y), 15)
...
def check_collisions(rects, circle, act):
    for i in range(len(rects)):
...
while run:
    ...
    active = check_collisions(map_rects, player_circle, active)
    ...
    map_speed = 2 + score // 50  # the bigger no we put the longer we wil be able to play before losing...
    spacer = 10 + score // 100
    screen.blit(font.render(f"Score : {score}", True, 'white'), (20, 20))
    screen.blit(font.render(f"HighScore : {highscore}", True, 'white'), (20, 560))
    if not active:
        screen.blit(font.render('Press ENTER to Restart', True, 'white'), (370, 20))
        screen.blit(font.render('Press ENTER to Restart', True, 'white'), (370, 560))
